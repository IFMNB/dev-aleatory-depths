...
function private.main (World)
	...
	mass:DoCatchFromWorld(World, function(ComMass) 
		private.fetch_mass(World, ComMass)
	end)
end

function private.fetch_mass (World, ComMass: typeof(mass.Value))
	model:DoCatchFromWorld(World, function(ComModel: { Path: string, Source: string }) 
		...
		if this then
			...
			if primary then
				ComMass.Previous = ComMass.Value
				ComMass.Value = primary.AssemblyMass
			end
		end
	end)
end
